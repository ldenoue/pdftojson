...
public:
  ...
  TextoutFormField(int xMinA, int yMinA, int xMaxA, int yMaxA,
	   const char *typeA, TextString *nameA, TextString *valueA, TextString *alternativeTextA)
    { xMin = xMinA; yMin = yMinA; xMax = xMaxA; yMax = yMaxA;
      ...
      //if (name == NULL)
      //  name = new GString();
      value = new TextString(valueA);
      //if (value == NULL)
      //  value = new GString();
      alternativeText = new TextString(alternativeTextA);
      //printf("| %s | %s | %s |\n",name->toPDFTextString()->getCString(),value->toPDFTextString()->getCString(),alternativeText->toPDFTextString()->getCString());
...
void TextPage::addTextoutFormField(int xmin, int ymin, int xmax, int ymax, const char* type, TextString *name, TextString *value, TextString *alttext)
{
  TextoutFormField *tf = new TextoutFormField(xmin,ymin,xmax,ymax,type,name,value,alttext);
  //printf("%s\n",tf->name->toPDFTextString()->getCString());
...
void TextOutputDev::processFormField(FormField *formfield) {
  ...
  if (res)
  {
    text->addTextoutFormField(xMin, yMin, xMax, yMax,
      formfield->getType(),
      formfield->getNameTS(),
      formfield->getValueTS(),
      formfield->getAltTextTS());
  }
}
...
static void appendTextString(TextString *word, GString *s)
{
  int i;
  Unicode u;
  int len = word->getLength();
  Unicode *unicode = word->getUnicode();
  if (unicode == NULL)
    return;
  for (i = 0; i < len; ++i) {
    u = unicode[i];
...
GString *TextOutputDev::getTextoutFormFields() {
  ...
  for (int i=0;i<len;i++)
  {
    ...
    //printf("%s %d\n",f->name->toPDFTextString()->getCString(),f->name->getLength());
    //res->appendf("[{0:d},{1:d},{2:d},{3:d},{4:d},\"{5:s}\",\"{6:s}\",\"{7:s}\",\"{8:s}\"]",f->yMin, f->xMin, (int)(f->xMax-f->xMin+2), (int)(f->yMax-f->yMin+2),page,f->type->getCString(),f->name,f->alternativeText,f->value);
    res->appendf("[{0:d},{1:d},{2:d},{3:d},{4:d},\"{5:s}\",\"",f->yMin, f->xMin, (int)(f->xMax-f->xMin+2), (int)(f->yMax-f->yMin+2),page,f->type->getCString());
    appendTextString(f->name,res);
    res->appendf("\",\"");
    appendTextString(f->value,res);
    res->appendf("\",\"");
    appendTextString(f->alternativeText,res);
    res->appendf("\"]");
    //{6:s}\",\"{7:s}\",\"{8:s}\"]",f->yMin, f->xMin, (int)(f->xMax-f->xMin+2), (int)(f->yMax-f->yMin+2),page,f->type->getCString(),f->name,f->alternativeText,f->value);
    if (i<len-1)
      res->append(",");
  }
  res->append("]");
